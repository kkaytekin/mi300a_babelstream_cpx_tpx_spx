#!/bin/bash
OUTDIR=/zhome/academic/HLRS/hlrs/hpckkuec/cpx_tpx_test/logs/010_tpx
BABELSTREAM=/lustre/hpe/ws13/ws13.a/ws/hpckkuec-babelstream/BabelStream/build/hip-stream
NUMTIMES=600

# TPX: 12 devices, each covering 2 XCDs -> arraysize = 2 * 2^exp
# Sweep bounds (powers of two, applied per XCD)
EXP_MIN=10
EXP_MAX=28
...
for exp in $(seq $EXP_MIN $EXP_MAX); do
  ...
  echo "=== Array size: 2 * 2^${exp} = ${arraysize} elements (${size_kib} KiB per array) ==="
...
for i in $(seq 0 11); do printf " dev%-2s" $i; done
echo ""
