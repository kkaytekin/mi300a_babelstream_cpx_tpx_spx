#!/bin/bash
OUTDIR=/zhome/academic/HLRS/hlrs/hpckkuec/cpx_tpx_test/logs/010_spx
BABELSTREAM=/lustre/hpe/ws13/ws13.a/ws/hpckkuec-babelstream/BabelStream/build/hip-stream
NUMTIMES=250

# SPX: 4 devices, each covering 6 XCDs -> arraysize = 6 * 2^exp
...
for exp in $(seq $EXP_MIN $EXP_MAX); do
  ...
  echo "=== Array size: 6 * 2^${exp} = ${arraysize} elements (${size_kib} KiB per array) ==="
...
for i in $(seq 0 3); do printf " dev%-2s" $i; done
echo ""
