...
Memory bandwidth is the silent bottleneck behind much of GPU computing. Everything ultimately funnels through the memory bus. And on a chip as architecturally novel as the AMD Instinct MI300A, how you *partition* the hardware can shift throughput by double-digit percentages.

This study runs [BabelStream](https://github.com/UoB-HPC/BabelStream), the standard HBM bandwidth benchmark, across all three MI300A partition modes (CPX, TPX, and SPX) on HLRS's [Hunter](https://www.hlrs.de/solutions/systems/hunter) supercomputer. The result is surprising: the "smallest" device configuration delivers the highest whole-node bandwidth. CPX mode, where every XCD operates as its own independent device, reaches **78% of theoretical peak** for the Triad kernel. TPX comes in at 70%, and SPX at 63%.
...
The MI300X is the pure-GPU sibling. Both variants share the same 3.5D packaging with **4 IODs** (I/O Dies) and **8 HBM3 stacks**, but differ in what sits on top:

- **MI300X:** 8 XCDs (no CPU cores), 12-Hi HBM3 stacks totaling **192 GB**, rated at **5.3 TB/s**
- **MI300A:** 6 XCDs + 3 CCDs (24 Zen 4 CPU cores), 8-Hi HBM3 stacks totaling **128 GB**, also rated at **5.3 TB/s**

The MI300A trades two XCDs for three CPU Complex Dies (CCDs), giving up raw GPU compute in exchange for unified CPU-GPU memory on the same package. Despite the smaller HBM stack height (8-Hi vs 12-Hi), both variants achieve the same 5.3 TB/s theoretical bandwidth per package. For a pure-GPU streaming benchmark like BabelStream, the MI300X's 8 XCDs driving 8 HBM stacks could be expected to achieve slightly higher aggregate bandwidth than the MI300A's 6 XCDs (where the CPU dies sit idle for compute). This comparison remains untested, as only MI300A hardware was available for this study.

![MI300X vs. MI300A](plotting/figures/mi300x_vs_mi300a.png)
...
Now every APU services ~38.4 GB total, regardless of partition mode. The buffer size sweep (discussed below) confirms that all these array sizes sit well above the ~100 MB saturation threshold, ensuring steady-state HBM throughput rather than cache effects.

### Calibrating run duration

Benchmark duration matters. Too short, and startup noise dominates. Too long, and cluster time is wasted. A calibration script (`find_numtimes.sh`) uses iterative extrapolation to find the `--numtimes` parameter that yields approximately 60 seconds of runtime:
...
The idea is simple: run a probe, measure wall-clock time, linearly extrapolate to the target, and repeat until convergence. This yields `numtimes=1500` for the standard 2 GB array (CPX/TPX) and proportionally lower values for larger arrays (e.g., `numtimes=600` for 4 GB in TPX, `numtimes=250` for ~13 GB in SPX).
...
Before jumping to the headline numbers, it helps to understand how bandwidth evolves as a function of problem size. A buffer size sweep from kilobytes to gigabytes across all three modes reveals the underlying scaling behavior.

### The S-curve

Starting with CPX, the classic shape emerges: bandwidth starts low (limited by launch overhead and caches), ramps steeply, and saturates once the working set exceeds the HBM's effective reach. The saturation threshold sits around **17 MB per array**, well below the 2+ GB arrays used in the full-node experiments.

![CPX Per-Device Bandwidth Sweep](plotting/figures/sweep_01_cpx_device.png)

### All three modes side by side

TPX and SPX follow the same S-curve pattern, but at different absolute scales, roughly proportional to XCDs per device:
- CPX (1 XCD): ~0.71 TB/s per device
- TPX (2 XCDs): ~1.35 TB/s per device
- SPX (6 XCDs): ~3.65 TB/s per device

Saturation occurs at similar buffer sizes across all modes (~100-130 MB), confirming that the full-node experiments measure steady-state HBM bandwidth, not cache effects.

![Per-Device Sweep: CPX / TPX / SPX](plotting/figures/sweep_02_all_modes_device.png)

### The key insight: silicon is silicon

What happens when bandwidth is normalized **per XCD** and plotted against per-XCD buffer size? The three mode curves **nearly overlap**. At the individual-XCD level, the silicon delivers similar bandwidth regardless of whether it's operating as an independent device (CPX), part of a pair (TPX), or part of a group of six (SPX).

![Per-XCD Normalised Triad](plotting/figures/sweep_03_per_xcd_normalized.png)

### Where modes diverge

But "nearly" isn't "exactly." At the aggregate level (per-APU and whole-node) the modes separate clearly. CPX sits highest, TPX in the middle, SPX at the bottom. Small per-XCD differences, multiplied by 6 XCDs per APU and 4 APUs per node, compound into a significant gap.

![Per-APU and Whole-Node Triad](plotting/figures/sweep_04_apu_and_node.png)

---

## Head-to-Head: CPX vs TPX vs SPX

Now for the main comparison. All devices run simultaneously on the full node, with **normalized array sizes** (each XCD processes 2^28 elements).

### The headline numbers

![Whole-Node Triad: CPX vs TPX vs SPX](plotting/figures/h2h_01_triad_headline.png)
...
The pattern holds for Copy, Mul, Add, and Triad: CPX leads, TPX is in the middle, SPX trails. But look at the efficiency curves on the right: all three modes track closely for the streaming kernels, then diverge sharply at Dot.

![All Five Kernels: Absolute + Efficiency](plotting/figures/h2h_02_all_kernels.png)
...
This makes intuitive sense. A Dot reduction on an SPX device must aggregate partial sums across 6 XCDs. In CPX mode, each device's Dot reduction happens entirely within a single XCD, requiring no cross-die communication.
...
This raises a natural question: taking CPX mode and running two XCDs on the same APU simultaneously, does that match what TPX delivers? In other words, is a TPX device just two CPX devices stapled together, or does the TPX device abstraction itself cost something?

To test this, BabelStream was launched in CPX mode on two adjacent devices belonging to the same APU (e.g., devices 0 and 1 on APU 0) and their bandwidth was summed. That was then compared against real TPX mode running on the corresponding single 2-XCD device.

![Real TPX vs Emulated TPX](plotting/figures/005_real_Tpx_vs_TpxEmulatedInCpx.png)
...
Does doubling the array size change anything? This was tested on a single APU in CPX mode, comparing 2^28 (2 GB) and 2^29 (4 GB) arrays.
...
![SPX Full Node](plotting/figures/007_spx_full_node.png)
...
![TPX Full Node](plotting/figures/006_tpx_full_node.png)
...
### What if array sizes are not normalized?

It's worth addressing this directly, since at first glance it might seem simpler to just use the same array size for all modes and compare. That experiment was also run, and the result initially suggested TPX was the winner. Understanding why that conclusion is misleading requires looking at what BabelStream actually does under the hood.
...
#### What happens with 6 processes per APU

In CPX mode, the test script launches 24 independent BabelStream processes, one per XCD, six per APU. Each process allocates its own 3 arrays (totaling ~6.4 GB), and streams through them independently. The six processes on the same APU all compete for the same HBM bandwidth pool.
...
When the reported bandwidths are summed to get "whole-node Triad," the computation is:
...
CPX is pushing **6x** the total data through each APU's HBM controllers. This is no longer comparing "which partition mode delivers more bandwidth" but rather "how well does the HBM subsystem hold up under 6x the load." That's a congestion test, not a bandwidth test.
...
When array sizes are normalized (as described in the experimental setup), every APU services ~38.4 GB regardless of partition mode, and CPX's advantage becomes clear.
...
**2. Multi-XCD device coordination is the likely culprit.** The emulated TPX experiment shows that two independent CPX devices outperform a single fused TPX device on identical hardware, pointing to overhead in coordinating XCDs within a device. Pinpointing the exact mechanism (runtime scheduling, memory management, firmware-level arbitration, or a combination) would require low-level profiling (e.g., `rocprof`) not yet performed in this study.
...
All results in this post come from a **single MI300A node** on the HLRS Hunter cluster. Node-to-node variation (due to silicon lottery on HBM stacks, firmware differences, or thermal conditions) could shift absolute numbers by several percent. This single node cannot be claimed as representative of all MI300A systems. The relative ordering of CPX > TPX > SPX is likely robust (it follows from structural differences in how XCDs are grouped), but the exact percentage gaps should be treated as indicative, not definitive. Reproducing these experiments across multiple nodes would strengthen the conclusions.
